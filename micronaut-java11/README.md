...
Was able to set up the Micronaut application quite quickly and found ample of support online. While I liked the
end-to-end test support, there is a small discrepancy between the Java test and the test event in AWS. In AWS, the body
needs to be passed as a `String`, with the quotes escaped.

The obtained results were quite promising. A request takes less than 100 milliseconds and cold starts are not as
painful, about 6 seconds.
